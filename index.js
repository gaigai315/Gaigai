...
新增人物:
<GaigaiMemory><!-- insertRow(3, {0: "艾莉娅", 1: "23", 2: "神秘商人", 3: "迷雾森林", 4: "神秘冷静，知识渊博", 5: "中立友好"})--></GaigaiMemory>
...
请严格按照以上格式生成总结。`,
    ...
    class SM {
        ...
        clear() { const sumSheet = this.m.get(7); sumSheet.clear(); this.m.save(); }
        has() { const sumSheet = this.m.get(7); return sumSheet.r.length > 0 && sumSheet.r[0][1]; }
        getTime() { const sumSheet = this.m.get(7); if (sumSheet.r.length > 0) { return sumSheet.r[0][2] || ''; } return ''; }
    }
    ...
    function getRoleByPosition(pos) { if (pos === 'system' || pos === 'world_info_before' || pos === 'world_info_after') return 'system'; return 'user'; }
    function getInjectionPosition(pos, depth, chatLength) {
        switch(pos) {
            ...
            case 'before_last': return Math.max(0, chatLength - 1 - depth);
            case 'world_info_before': return 1 + depth;
            case 'world_info_after': return 2 + depth;
    ...
    function shw() {
        pageStack = [shw];
        const ss = m.all();
        const tbs = ss.slice(0, 7).map((s, i) => { const count = s.r.length; return `<button class="g-t${i === 0 ? ' act' : ''}" data-i="${i}">${s.n} (${count})</button>`; }).join('');
        const tls = `<input type="text" id="g-src" placeholder="搜索"><button id="g-ad" title="新增行">➕ 新增</button><button id="g-dr" title="删除选中行" style="background:#dc3545;">🗑️ 删除选中</button><button id="g-sm" title="生成AI总结">📝 总结</button><button id="g-ex" title="导出数据">📥 导出</button><button id="g-ca" title="清空所有表格">🗑️ 全清</button><button id="g-tm" title="主题设置">🎨</button><button id="g-cf" title="配置">⚙️</button>`;
    ...
    function gtb(s, ti) {
        ...
        if (s.r.length === 0) {
            ...
            s.r.forEach((rw, ri) => {
                ...
                s.c.forEach((c, ci) => { const val = rw[ci] || ''; h += `<td><div class="g-e" contenteditable="true" data-r="${ri}" data-c="${ci}">${esc(val)}</div></td>`; });
                h += '</tr>';
            });
        }
    ...
    async function callAIForSummary() {
        ...
        if (tables.length === 0) return { success: false, error: '没有表格数据' };
        if (!API_CONFIG.enableAI) return { success: false, error: '请先启用AI总结功能' };
        let prompt = PROMPTS.summaryPrompt + '\n\n';
        tables.forEach(s => { prompt += s.txt() + '\n\n'; });
        try {
            if (API_CONFIG.useIndependentAPI) {
                if (!API_CONFIG.apiKey) return { success: false, error: '请先配置独立API密钥' };
                let response;
                if (API_CONFIG.provider === 'gemini') {
                    response = await fetch(`${API_CONFIG.apiUrl}?key=${API_CONFIG.apiKey}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: prompt }] }] }) });
                    if (response.ok) { const data = await response.json(); const summary = data.candidates[0].content.parts[0].text; return { success: true, summary }; }
                } else {
                    response = await fetch(API_CONFIG.apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${API_CONFIG.apiKey}` }, body: JSON.stringify({ model: API_CONFIG.model, messages: [{ role: 'user', content: prompt }], temperature: API_CONFIG.temperature, max_tokens: API_CONFIG.maxTokens }) });
                    if (response.ok) { const data = await response.json(); const summary = data.choices[0].message.content; return { success: true, summary }; }
                }
                const error = await response.text();
                return { success: false, error: `HTTP ${response.status}: ${error}` };
            } else {
                const context = m.ctx();
                if (!context || !context.generate) return { success: false, error: '无法访问酒馆API，请确保酒馆已连接' };
                try { const summary = await context.generate(prompt, { max_tokens: 1000, temperature: 0.7 }); if (summary) return { success: true, summary }; else return { success: false, error: '酒馆API未返回内容' }; } catch (err) { return { success: false, error: `酒馆API调用失败: ${err.message}` }; }
            }
        } catch (e) { return { success: false, error: e.message }; }
    }
        function genSummary() {
        const hasData = m.all().slice(0, 7).some(s => s.r.length > 0);
        if (!hasData) { alert('⚠️ 没有表格数据，无法生成总结'); return; }
        const currentSummary = m.sm.load();
        const canUseAI = API_CONFIG.enableAI && (API_CONFIG.useIndependentAPI ? !!API_CONFIG.apiKey : true);
        const h = `<div class="g-p"><h4>📝 生成AI总结</h4>${m.sm.has() ? `<div style="background:#d4edda; padding:10px; border-radius:4px; margin-bottom:12px;"><strong>✅ 当前已有总结</strong><br><small style="color:#666;">生成时间：${m.sm.getTime()}</small><br><small style="color:#666;">共 ${m.sm.loadArray().length} 条</small></div>` : ''}<label style="font-weight:600; margin-bottom:8px; display:block;">总结内容（每行一条）：</label><textarea id="sum-txt" style="width:100%; height:250px; padding:8px; border:1px solid #ddd; border-radius:4px; font-size:10px; font-family:monospace; resize:vertical;" placeholder="每行一条总结，例如：\n• 主线剧情：2024年3月15日，接受长老委托前往迷雾森林寻找失落宝石\n• 人物档案：新认识神秘商人艾莉娅（23岁）...">${esc(currentSummary)}</textarea><div style="margin-top:12px;">${canUseAI ? `<button id="ai-sum" style="padding:6px 12px; background:#17a2b8; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:11px;">🤖 调用AI生成${API_CONFIG.useIndependentAPI ? '（独立API）' : '（酒馆API）'}</button>` : '<button disabled style="padding:6px 12px; background:#ccc; color:#666; border:none; border-radius:4px; cursor:not-allowed; font-size:11px;" title="请先在配置中启用AI总结">🤖 调用AI（未配置）</button>'}<button id="copy-data" style="padding:6px 12px; background:#28a745; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:11px;">📋 复制表格数据</button></div><hr style="margin:12px 0; border:none; border-top:1px solid #ddd;"><div style="background:#fff3cd; padding:10px; border-radius:4px; font-size:10px; margin-bottom:12px;"><strong>💡 使用提示：</strong><br>${API_CONFIG.enableAI ? `• 方式1：点击"调用AI生成"自动生成分条总结（使用${API_CONFIG.useIndependentAPI ? '独立' : '酒馆'}API）<br>` : '• 方式1：请先在配置中启用AI总结功能<br>'}• 方式2：复制表格数据，在聊天中让AI总结，再粘贴回来<br>• 每行一条总结，支持 • 或数字开头<br>• 保存后，总结会和表格一起发送给AI</div><button id="save-sum" style="padding:6px 12px; background:${UI.c}; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:11px;">💾 保存总结</button><button id="clear-sum" style="padding:6px 12px; background:#dc3545; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:11px;">🗑️ 删除总结</button></div>`;
        pop('📝 生成AI总结', h, true);
        setTimeout(() => {
            $('#ai-sum').on('click', async function() {
                ...
                try {
                    ...
                    if (result.success) { $('#sum-txt').val(result.summary); alert('✅ AI总结生成成功！'); } else { alert('❌ 生成失败：' + result.error); }
            ...
            $('#copy-data').on('click', function() {
                ...
                navigator.clipboard.writeText(txt).then(() => {
                    ...
                    setTimeout(() => { $(this).text('📋 复制表格数据').css('background', '#28a745'); }, 2000);
                });
            });
            $('#save-sum').on('click', function() { const txt = $('#sum-txt').val().trim(); if (!txt) { alert('请输入总结内容'); return; } m.sm.save(txt); alert('✅ 总结已保存到"记忆总结"表格'); goBack(); });
            $('#clear-sum').on('click', function() { if (!confirm('确定删除总结？')) return; m.sm.clear(); alert('✅ 总结已删除'); goBack(); });
        }, 100);
    }
    function shtm() {
    ...
    function shapi() {
        const h = `<div class="g-p"><h4>🤖 AI 总结配置</h4><fieldset style="border:1px solid #ddd; padding:10px; border-radius:4px; margin-bottom:12px;"><legend style="font-size:11px; font-weight:600;">基础设置</legend><label><input type="checkbox" id="api-enable" ${API_CONFIG.enableAI ? 'checked' : ''}> 启用AI自动总结</label><p style="font-size:10px; color:#666; margin:4px 0 0 20px;">开启后可使用AI生成表格总结</p><br><label><input type="checkbox" id="api-independent" ${API_CONFIG.useIndependentAPI ? 'checked' : ''}> 使用独立API（不同于酒馆）</label><p style="font-size:10px; color:#666; margin:4px 0 0 20px;">✅ 不勾选：使用酒馆当前连接的API（无需配置）<br>✅ 勾选：使用下方配置的独立API</p></fieldset><fieldset id="api-config-section" style="border:1px solid #ddd; padding:10px; border-radius:4px; margin-bottom:12px; ${API_CONFIG.useIndependentAPI ? '' : 'opacity:0.5; pointer-events:none;'}"><legend style="font-size:11px; font-weight:600;">独立API配置 ${API_CONFIG.useIndependentAPI ? '' : '（已禁用）'}</legend><label>API提供商：</label><select id="api-provider" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px; margin-bottom:10px;"><option value="openai" ${API_CONFIG.provider === 'openai' ? 'selected' : ''}>OpenAI</option><option value="gemini" ${API_CONFIG.provider === 'gemini' ? 'selected' : ''}>Google Gemini</option><option value="openai-compatible" ${API_CONFIG.provider === 'openai-compatible' ? 'selected' : ''}>兼容OpenAI格式</option></select><label>API地址（URL）：</label><input type="text" id="api-url" value="${API_CONFIG.apiUrl}" placeholder="https://api.openai.com/v1/chat/completions" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px; font-size:10px; margin-bottom:10px;"><label>API密钥（Key）：</label><input type="password" id="api-key" value="${API_CONFIG.apiKey}" placeholder="sk-..." style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px; font-size:10px; margin-bottom:10px;"><label>模型名称：</label><input type="text" id="api-model" value="${API_CONFIG.model}" placeholder="gpt-3.5-turbo" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px; font-size:10px; margin-bottom:10px;"><label>温度（Temperature）：<span id="api-temp-val">${API_CONFIG.temperature}</span></label><input type="range" id="api-temp" min="0" max="2" step="0.1" value="${API_CONFIG.temperature}" style="width:100%; margin-bottom:10px;"><label>最大Token数：</label><input type="number" id="api-tokens" value="${API_CONFIG.maxTokens}" min="100" max="32000" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px;"></fieldset><div style="background:#e7f3ff; padding:10px; border-radius:4px; font-size:10px; margin-bottom:12px;"><strong>💡 使用说明：</strong><br><strong>场景1：使用酒馆API（推荐）</strong><br>• 只勾选"启用AI自动总结"<br>• 不勾选"使用独立API"<br>• 插件会自动使用酒馆当前连接的API<br><br><strong>场景2：使用独立API</strong><br>• 同时勾选两个选项<br>• 配置下方的API参数<br>• 填表和聊天使用不同的API</div><button id="save-api" style="padding:6px 12px; background:${UI.c}; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:11px;">💾 保存</button><button id="test-api" style="padding:6px 12px; background:#17a2b8; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:11px;" ${API_CONFIG.useIndependentAPI ? '' : 'disabled'}>🧪 测试连接</button></div>`;
        pop('🤖 AI 总结配置', h, true);
        setTimeout(() => {
            $('#api-independent').on('change', function() {
                const isIndependent = $(this).is(':checked');
                if (isIndependent) { $('#api-config-section').css({'opacity': '1', 'pointer-events': 'auto'}); $('#api-config-section legend').text('独立API配置'); $('#test-api').prop('disabled', false); } else { $('#api-config-section').css({'opacity': '0.5', 'pointer-events': 'none'}); $('#api-config-section legend').text('独立API配置（已禁用）'); $('#test-api').prop('disabled', true); }
            });
            $('#api-temp').on('input', function() { $('#api-temp-val').text($(this).val()); });
            $('#api-provider').on('change', function() { const provider = $(this).val(); if (provider === 'openai') { $('#api-url').val('https://api.openai.com/v1/chat/completions'); $('#api-model').val('gpt-3.5-turbo'); } else if (provider === 'gemini') { $('#api-url').val('https://generativelanguage.googleapis.com/v1beta/models/gemini-pro:generateContent'); $('#api-model').val('gemini-pro'); } });
            $('#save-api').on('click', function() { API_CONFIG.enableAI = $('#api-enable').is(':checked'); API_CONFIG.useIndependentAPI = $('#api-independent').is(':checked'); API_CONFIG.provider = $('#api-provider').val(); API_CONFIG.apiUrl = $('#api-url').val(); API_CONFIG.apiKey = $('#api-key').val(); API_CONFIG.model = $('#api-model').val(); API_CONFIG.temperature = parseFloat($('#api-temp').val()); API_CONFIG.maxTokens = parseInt($('#api-tokens').val()); try { localStorage.setItem(AK, JSON.stringify(API_CONFIG)); } catch (e) {} alert('✅ API配置已保存'); });
            $('#test-api').on('click', async function() { if (!API_CONFIG.useIndependentAPI) { alert('⚠️ 请先勾选"使用独立API"'); return; } const btn = $(this); btn.text('测试中...').prop('disabled', true); try { const result = await testAPIConnection(); if (result.success) { alert('✅ API连接成功！\n\n' + result.message); } else { alert('❌ API连接失败\n\n' + result.error); } } catch (e) { alert('❌ 测试出错：' + e.message); } btn.text('🧪 测试连接').prop('disabled', false); });
        }, 100);
    }
    async function testAPIConnection() {
        const config = { provider: $('#api-provider').val(), apiUrl: $('#api-url').val(), apiKey: $('#api-key').val(), model: $('#api-model').val() };
        if (!config.apiKey) return { success: false, error: '请输入API密钥' };
        try {
            let response;
            if (config.provider === 'gemini') { response = await fetch(`${config.apiUrl}?key=${config.apiKey}`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ contents: [{ parts: [{ text: 'Hello' }] }] }) }); } else { response = await fetch(config.apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${config.apiKey}` }, body: JSON.stringify({ model: config.model, messages: [{ role: 'user', content: 'Hello' }], max_tokens: 10 }) }); }
            if (response.ok) return { success: true, message: 'API连接正常，模型响应成功' }; else { const error = await response.text(); return { success: false, error: `HTTP ${response.status}: ${error}` }; }
        } catch (e) { return { success: false, error: e.message }; }
    }
    function shpmt() {
        const h = `<div class="g-p"><h4>📝 提示词管理</h4><p style="color:#666; font-size:11px; margin-bottom:10px;">配置AI填表和生成总结的提示词模板。</p><fieldset style="border:1px solid #ddd; padding:10px; border-radius:4px; margin-bottom:12px;"><legend style="font-size:11px; font-weight:600;">填表提示词</legend><label>提示词内容：</label><textarea id="pmt-table" style="width:100%; height:300px; padding:8px; border:1px solid #ddd; border-radius:4px; font-size:10px; font-family:monospace; resize:vertical; margin-bottom:10px;">${esc(PROMPTS.tablePrompt)}</textarea><label>注入位置：</label><select id="pmt-table-pos" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px; margin-bottom:10px;"><option value="system" ${PROMPTS.tablePromptPos === 'system' ? 'selected' : ''}>系统消息</option><option value="user" ${PROMPTS.tablePromptPos === 'user' ? 'selected' : ''}>用户消息</option><option value="assistant" ${PROMPTS.tablePromptPos === 'assistant' ? 'selected' : ''}>助手消息</option><option value="world_info_before" ${PROMPTS.tablePromptPos === 'world_info_before' ? 'selected' : ''}>世界书之前</option><option value="world_info_after" ${PROMPTS.tablePromptPos === 'world_info_after' ? 'selected' : ''}>世界书之后</option></select><label>注入深度：</label><input type="number" id="pmt-table-depth" value="${PROMPTS.tablePromptDepth}" min="0" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px;"><p style="font-size:10px; color:#666; margin:4px 0 0 0;">深度表示从指定位置往前偏移的消息数</p></fieldset><fieldset style="border:1px solid #ddd; padding:10px; border-radius:4px; margin-bottom:12px;"><legend style="font-size:11px; font-weight:600;">总结提示词</legend><label>提示词内容：</label><textarea id="pmt-summary" style="width:100%; height:200px; padding:8px; border:1px solid #ddd; border-radius:4px; font-size:10px; font-family:monospace; resize:vertical; margin-bottom:10px;">${esc(PROMPTS.summaryPrompt)}</textarea><label>注入位置：</label><select id="pmt-summary-pos" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px; margin-bottom:10px;"><option value="system" ${PROMPTS.summaryPromptPos === 'system' ? 'selected' : ''}>系统消息</option><option value="user" ${PROMPTS.summaryPromptPos === 'user' ? 'selected' : ''}>用户消息</option><option value="assistant" ${PROMPTS.summaryPromptPos === 'assistant' ? 'selected' : ''}>助手消息</option><option value="world_info_before" ${PROMPTS.summaryPromptPos === 'world_info_before' ? 'selected' : ''}>世界书之前</option><option value="world_info_after" ${PROMPTS.summaryPromptPos === 'world_info_after' ? 'selected' : ''}>世界书之后</option></select><label>注入深度：</label><input type="number" id="pmt-summary-depth" value="${PROMPTS.summaryPromptDepth}" min="0" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px;"></fieldset><div style="background:#fff3cd; padding:10px; border-radius:4px; font-size:10px; margin-bottom:12px;"><strong>💡 使用说明：</strong><br>• <strong>填表提示词：</strong>指导AI如何生成 &lt;GaigaiMemory&gt; 标签<br>• <strong>总结提示词：</strong>指导AI如何总结表格数据（分条格式）<br>• 两个提示词会分别注入到对话中<br>• 注入位置和深度可独立设置</div><button id="save-pmt" style="padding:6px 12px; background:${UI.c}; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:11px;">💾 保存</button><button id="reset-pmt" style="padding:6px 12px; background:#6c757d; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:11px;">🔄 恢复默认</button></div>`;
        pop('📝 提示词管理', h, true);
        setTimeout(() => {
            $('#save-pmt').on('click', function() { PROMPTS.tablePrompt = $('#pmt-table').val(); PROMPTS.tablePromptPos = $('#pmt-table-pos').val(); PROMPTS.tablePromptDepth = parseInt($('#pmt-table-depth').val()) || 0; PROMPTS.summaryPrompt = $('#pmt-summary').val(); PROMPTS.summaryPromptPos = $('#pmt-summary-pos').val(); PROMPTS.summaryPromptDepth = parseInt($('#pmt-summary-depth').val()) || 0; try { localStorage.setItem(PK, JSON.stringify(PROMPTS)); } catch (e) {} alert('✅ 提示词已保存'); });
            $('#reset-pmt').on('click', function() { if (!confirm('确定恢复为默认提示词？')) return; PROMPTS.tablePromptPos = 'system'; PROMPTS.tablePromptDepth = 0; PROMPTS.summaryPromptPos = 'system'; PROMPTS.summaryPromptDepth = 1; $('#pmt-table-pos').val('system'); $('#pmt-table-depth').val('0'); $('#pmt-summary-pos').val('system'); $('#pmt-summary-depth').val('1'); alert('✅ 提示词位置已重置'); });
        }, 100);
    }
    function shcf() {
        const h = `<div class="g-p"><h4>⚙️ 高级配置</h4><fieldset style="border:1px solid #ddd; padding:10px; border-radius:4px; margin-bottom:12px;"><legend style="font-size:11px; font-weight:600;">表格数据注入</legend><label><input type="checkbox" id="c-table-inj" ${C.tableInj ? 'checked' : ''}> 启用表格数据注入</label><br><br><label>注入位置：</label><select id="c-table-pos" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px;"><option value="system" ${C.tablePos === 'system' ? 'selected' : ''}>系统消息</option><option value="user" ${C.tablePos === 'user' ? 'selected' : ''}>用户消息</option><option value="assistant" ${C.tablePos === 'assistant' ? 'selected' : ''}>助手消息</option><option value="world_info_before" ${C.tablePos === 'world_info_before' ? 'selected' : ''}>世界书之前</option><option value="world_info_after" ${C.tablePos === 'world_info_after' ? 'selected' : ''}>世界书之后</option></select><br><br><label>深度：</label><input type="number" id="c-table-depth" value="${C.tableDepth}" min="0" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px;"><p style="font-size:10px; color:#666; margin:4px 0 0 0;">表格数据包含总结和详细表格</p></fieldset><fieldset style="border:1px solid #ddd; padding:10px; border-radius:4px; margin-bottom:12px;"><legend style="font-size:11px; font-weight:600;">自动总结</legend><label><input type="checkbox" id="c-auto-sum" ${C.autoSummary ? 'checked' : ''}> 启用自动总结</label><br><br><label>触发楼层数：</label><input type="number" id="c-auto-floor" value="${C.autoSummaryFloor}" min="10" style="width:100%; padding:5px; border:1px solid #ddd; border-radius:4px;"><p style="font-size:10px; color:#666; margin:4px 0 0 0;">当消息数达到此值时，自动调用AI生成总结</p></fieldset><fieldset style="border:1px solid #ddd; padding:10px; border-radius:4px; margin-bottom:12px;"><legend style="font-size:11px; font-weight:600;">云同步</legend><label><input type="checkbox" id="c-cloud" ${C.cloudSync ? 'checked' : ''}> 启用云同步</label><p style="font-size:10px; color:#666; margin:4px 0 0 20px;">数据保存到聊天元数据中，随聊天记录同步</p></fieldset><fieldset style="border:1px solid #ddd; padding:10px; border-radius:4px; margin-bottom:12px;"><legend style="font-size:11px; font-weight:600;">功能入口</legend><button id="open-api" style="padding:6px 12px; background:#17a2b8; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:11px; margin-right:5px;">🤖 AI总结配置</button><button id="open-pmt" style="padding:6px 12px; background:#17a2b8; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:11px;">📝 提示词管理</button></fieldset><fieldset style="border:1px solid #ddd; padding:10px; border-radius:4px; margin-bottom:12px;"><legend style="font-size:11px; font-weight:600;">其他选项</legend><label><input type="checkbox" id="c-log" ${C.log ? 'checked' : ''}> 控制台详细日志</label><br><br><label><input type="checkbox" id="c-pc" ${C.pc ? 'checked' : ''}> 每个角色独立数据</label><br><br><label><input type="checkbox" id="c-hide" ${C.hideTag ? 'checked' : ''}> 隐藏聊天中的记忆标签</label><br><br><label><input type="checkbox" id="c-filter" ${C.filterHistory ? 'checked' : ''}> 自动过滤历史标签</label></fieldset><button id="save-cfg" style="padding:8px 16px; background:${UI.c}; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:12px;">💾 保存配置</button></div>`;
        pop('⚙️ 配置', h, true);
        setTimeout(() => {
            $('#save-cfg').on('click', function() { C.tableInj = $('#c-table-inj').is(':checked'); C.tablePos = $('#c-table-pos').val(); C.tableDepth = parseInt($('#c-table-depth').val()) || 0; C.autoSummary = $('#c-auto-sum').is(':checked'); C.autoSummaryFloor = parseInt($('#c-auto-floor').val()) || 50; C.cloudSync = $('#c-cloud').is(':checked'); C.log = $('#c-log').is(':checked'); C.pc = $('#c-pc').is(':checked'); C.hideTag = $('#c-hide').is(':checked'); C.filterHistory = $('#c-filter').is(':checked'); alert('✅ 配置已保存'); });
    ...
    function omsg(id) {
        try {
            ...
            if (C.autoSummary && x.chat.length >= C.autoSummaryFloor && !m.sm.has()) {
                ...
                if (API_CONFIG.enableAI) { callAIForSummary().then(result => { if (result.success) { m.sm.save(result.summary); console.log('✅ [AUTO SUMMARY] 总结生成并保存成功'); } }); }
            }
    ...
    setTimeout(ini, 1000);
    window.Gaigai = { v: V, m: m, shw: shw, genSummary: genSummary, cleanMemoryTags: cleanMemoryTags, MEMORY_TAG_REGEX: MEMORY_TAG_REGEX, config: API_CONFIG, prompts: PROMPTS };
})();
