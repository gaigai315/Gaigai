...
(function() {
    ...
    const C = { 
        tableInj: true,
        tablePos: 'system',
        tablePosType: 'absolute', // 新增：absolute 或 chat
        tableDepth: 0,
        autoSummary: false,
        autoSummaryFloor: 50,
        log: true, 
...
【表格索引】
0: 主线剧情 (日期, 开始时间, 完结时间, 地点, 事件概要, 状态)
...
新增主线（新的一天）:
<GaigaiMemory><!-- insertRow(0, {0: "2024年3月15日", 1: "上午(08:30)", 2: "", 3: "迷雾森林", 4: "接受长老委托，前往迷雾森林寻找失落宝石", 5: "进行中"})--></GaigaiMemory>

更新主线（同一天新事件）:
<GaigaiMemory><!-- updateRow(0, 0, {4: "接受长老委托，前往迷雾森林寻找失落宝石；在迷雾森林遭遇神秘商人，获得线索：宝石在古神殿深处"})--></GaigaiMemory>

跨天处理（完结前一天，开始新一天）:
<GaigaiMemory><!-- updateRow(0, 0, {2: "深夜(23:50)", 5: "暂停"})
insertRow(0, {0: "2024年3月16日", 1: "凌晨(00:10)", 2: "", 3: "古神殿", 4: "继续深入古神殿探索", 5: "进行中"})--></GaigaiMemory>

新增支线:
<GaigaiMemory><!-- insertRow(1, {0: "进行中", 1: "艾莉娅的委托", 2: "2024年3月15日·下午(14:00)", 3: "", 4: "艾莉娅请求帮忙寻找失散的妹妹", 5: "艾莉娅"})--></GaigaiMemory>

新增人物:
<GaigaiMemory><!-- insertRow(3, {0: "艾莉娅", 1: "23", 2: "神秘商人", 3: "迷雾森林", 4: "神秘冷静，知识渊博", 5: "有一个失散的妹妹"})--></GaigaiMemory>

新增人物关系:
<GaigaiMemory><!-- insertRow(4, {0: "{{user}}", 1: "艾莉娅", 2: "委托人与委托者", 3: "中立友好"})--></GaigaiMemory>

新增承诺约定:
<GaigaiMemory><!-- insertRow(7, {0: "2024年3月18日前", 1: "找到失落宝石交给长老", 2: "长老"})--></GaigaiMemory>

【记录规则】
- 主线: 按日期记录{{char}}与{{user}}的重要互动，同一天事件用分号连接
- 支线: 仅记录NPC相关情节，命名格式"人物+目标"，必须标注完结状态
- 角色状态: 仅记录死亡/囚禁/残废等重大变化
- 人物档案: 仅记录世界书中不存在的新角色，备注可自由记录特征/背景等
- 人物关系: 仅记录决定性转换(朋友→敌人、陌生→恋人等)，情感态度描述双方情绪
- 世界设定: 仅记录世界书中不存在的新设定
- 物品追踪: 仅记录剧情关键物品
- 承诺约定: 记录重要约定和承诺

【强制要求】
1. 必须使用<GaigaiMemory>标签在最外层
2. 指令必须用<!-- -->包裹
3. 列索引从0开始: {0: "值", 1: "值"}
4. 同日事件用分号连接
5. 全部使用过去式，客观描述
...
• 人物档案：新认识神秘商人艾莉娅（23岁），性格神秘冷静，知识渊博
...
请严格按照以上格式生成总结。`,
    ...
    // ✅ 表格结构
    const T = [
        { n: '主线剧情', c: ['日期', '开始时间', '完结时间', '地点', '事件概要', '状态'] },
    ...
    function shw() {
        pageStack = [shw];
        const ss = m.all();
        const tbs = ss.slice(0, 8).map((s, i) => { const count = s.r.length; return `<button class="g-t${i === 0 ? ' act' : ''}" data-i="${i}">${s.n} (${count})</button>`; }).join('');
        const tls = `<input type="text" id="g-src" placeholder="搜索"><button id="g-ad" title="新增行">➕ 新增</button><button id="g-dr" title="删除选中行" style="background:#dc3545;">🗑️ 删除选中</button><button id="g-sm" title="总结表格">📝 总结</button><button id="g-ex" title="导出数据">📥 导出</button><button id="g-ca" title="清空所有表格">🗑️ 全清</button><button id="g-tm" title="主题设置">🎨</button><button id="g-cf" title="配置">⚙️</button>`;
        const tbls = ss.slice(0, 8).map((s, i) => gtb(s, i)).join('');
    ...
    function bnd() {
        ...
        $('#g-sm').on('click', () => navTo('总结表格', showSummaryView));
    ...
    function showSummaryView() {
        const sumSheet = m.get(8);
        const h = `<div class="g-vw" style="height:100%;"><div style="padding:10px; border-bottom:1px solid #ddd; background:#f8f9fa;"><h4 style="margin:0 0 10px 0;">📝 记忆总结表格</h4><p style="font-size:11px; color:#666; margin:0;">点击下方按钮生成总结，总结会保存到此表格中</p></div><div style="flex:1; overflow:auto; padding:10px;">${gtb(sumSheet, 8)}</div><div style="padding:10px; border-top:1px solid #ddd; background:#f8f9fa;"><button id="gen-sum-btn" style="padding:8px 16px; background:#28a745; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:12px; margin-right:8px;">🤖 生成总结</button><button id="clear-sum-btn" style="padding:8px 16px; background:#dc3545; color:#fff; border:none; border-radius:4px; cursor:pointer; font-size:12px;">🗑️ 清空总结</button></div></div>`;
        pop('📝 记忆总结', h, true);
        setTimeout(() => {
            $('#gen-sum-btn').on('click', callAIForSummary);
            $('#clear-sum-btn').on('click', function() {
                if (!confirm('确定清空总结表格？')) return;
                m.sm.clear();
                alert('✅ 总结已清空');
                showSummaryView();
            });
